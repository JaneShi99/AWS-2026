"""
complexity_vs_genus.sage — Benchmark compute_A_f_avg_poly_from_curve across
a range of genera, using random even-degree (2g+2) hyperelliptic curves.

Usage (run from repo root):
    sage scripts/complexity_vs_genus.sage <g_min> <g_max> <n_per_genus> <N_max>
...
Notes:
    ...
    - If that p is a bad prime for a curve (p | f(0)), a new curve is drawn
      automatically.
    - The plot is saved to scripts/complexity_vs_genus.png.
    - A power-law fit T(g) ≈ A·g^α is printed and overlaid on the plot.

Examples:
    sage scripts/complexity_vs_genus.sage 1 30 2 41
    sage scripts/complexity_vs_genus.sage 1 100 2 101
    sage scripts/complexity_vs_genus.sage 10 50 3 200
"""
...
USAGE = (
    "Usage: sage scripts/complexity_vs_genus.sage "
...
# ---------------------------------------------------------------------------
# Load algorithm and curve generator
# ---------------------------------------------------------------------------
...
def benchmark_genus(g, N):
    ...
    for all primes up to N, and return a list of process-time measurements
    in nanoseconds.
    ...
    for _ in range(N_PER_GENUS):
        while True:
            ...
            if p_check in result:
                ...
                break
            # p_check is a bad prime for this curve — retry

    return times_ns


print(f"Benchmarking even-degree (2g+2) curves")
...
fig.suptitle(
    rf"Average poly-time $A_f$ algorithm: process time vs genus"
    f"\n(even degree $2g+2$,  $N={N_MAX}$,  {N_PER_GENUS} curves/genus)",
    fontsize=12,
)

# Left: linear axes
ax = axes[0]
ax.scatter(genera, times_ms, color="steelblue", s=30, zorder=3, label="Measured")
ax.plot(g_fine, fit_ms, "--", color="steelblue", linewidth=1.5, alpha=0.8,
...
# Right: log-log axes (power law appears as a straight line)
ax = axes[1]
ax.scatter(g_arr, times_ms, color="steelblue", s=30, zorder=3, label="Measured")
ax.plot(g_fine, fit_ms, "--", color="steelblue", linewidth=1.5, alpha=0.8,
        label=rf"Fit: $T \propto g^{{{alpha:.2f}}}$")
ax.set_xscale("log")
ax.set_yscale("log")
...
outfile = f"scripts/complexity_vs_genus_{timestamp}.png"
